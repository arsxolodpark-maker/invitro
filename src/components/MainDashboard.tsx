...
export const MainDashboard: React.FC<MainDashboardProps> = ({ incidents, currentRole, onCreateIncident, onSelectIncident }) => {
  ...
  const filtered = useMemo(() => incidents.filter((inc) => {
    ...
    const matches = !q || [inc.id, inc.client, inc.clientCode, inc.lpu, inc.vendor, inc.inz].some((v) => v.toLowerCase().includes(q));
  ...
  const title = currentRole === 'Инженер ГСТИ' ? 'Очередь обращений' : 'Обращения';
  ...
  return (
    <div className="space-y-6 pb-12 notranslate" translate="no">
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
        ...
        {canCreate && <button type="button" onClick={onCreateIncident} className="inline-flex items-center justify-center gap-2 bg-[#0099a8] hover:bg-[#008590] text-white font-bold px-5 py-2.5 rounded-xl text-sm"><PlusCircle className="w-5 h-5" />Создать обращение</button>}
      </div>
      ...
      <div className="bg-white rounded-2xl border border-[#dfeaea] shadow-xs overflow-hidden">
        ...
        <div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead><tr className="bg-[#f6fbfb] text-xs uppercase tracking-wider text-slate-500"><th className="p-4">Обращение</th><th className="p-4">Клиент</th><th className="p-4">ИНЗ</th><th className="p-4">Статус</th><th className="p-4">1C:ITILIUM</th></tr></thead><tbody className="divide-y divide-[#edf3f3]">{filtered.map((incident) => <tr key={incident.id} onClick={() => onSelectIncident(incident.id)} className="hover:bg-[#f5fbfb] cursor-pointer" tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') onSelectIncident(incident.id); }}><td className="p-4"><div className="font-bold text-[#17383d]">{incident.id}</div><div className="text-xs text-slate-500">INC-02 · Не получен результат</div></td><td className="p-4"><div className="font-semibold">{incident.client}</div><div className="text-xs text-slate-500">{incident.lpu}</div></td><td className="p-4 font-mono text-xs">{incident.inz}</td><td className="p-4"><span className={`inline-flex px-2.5 py-1 rounded-full text-xs font-bold border ${statusClass(incident.status)}`}>{incident.status}</span></td><td className="p-4 text-xs text-slate-500">{incident.internalServiceDeskId || 'Регистрация · DEMO'}</td></tr>)}{filtered.length === 0 && <tr><td colSpan={5} className="p-10 text-center"><div className="font-bold text-[#17383d]">Ничего не найдено</div><div className="text-xs text-slate-500 mt-1">Измените запрос или сбросьте фильтр статуса.</div></td></tr>}</tbody></table></div>
      </div>
    </div>
  );
};
