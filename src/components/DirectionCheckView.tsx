...
import { AlertTriangle, ArrowRight, CheckCircle2, ChevronDown, CircleHelp, Database, FlaskConical } from 'lucide-react';
...
const stageStatus: Record<StageState, string> = { ok: 'Пройден', error: 'Ошибка', pending: 'Ожидает' };
const scenarioLabels: Record<DemoScenarioId, string> = { S1: 'Успешный путь', S2: 'Нет маппинга услуги', S3: 'Нечекин / нет маппинга услуги', S4: 'Ошибка доставки', S5: 'Не найдено' };
...
export const DirectionCheckView: React.FC<DirectionCheckViewProps> = ({ onCreateIncident }) => {
  ...
  const clearResult = () => { setResult(null); setNotFound(false); setValidationMessage(''); setExpandedStage(null); };

  const handleSearch = () => {
    if (!integration) {
      setValidationMessage('Выберите интеграцию.'); setResult(null); setNotFound(false);
      requestAnimationFrame(() => integrationRef.current?.focus());
      return;
    }
    if (!barcode.trim()) {
      setValidationMessage('Введите номер направления или штрихкод.'); setResult(null); setNotFound(false);
      requestAnimationFrame(() => barcodeRef.current?.focus());
      return;
    }
    setValidationMessage(''); setExpandedStage(null);
  ...
  const selectScenario = (scenarioId: DemoScenarioId) => {
    ...
    setIntegration(scenario.integration); setBarcode(scenario.barcode); setValidationMessage(''); setExpandedStage(null);
  ...
  const createIncident = (direction: DemoDirection) => {
    ...
    onCreateIncident({ source: 'GOVIN-303', client: direction.client || undefined, clientCode: direction.clientCode || undefined, lpu: direction.lpu || undefined, vendor: direction.integration, inz: direction.inz.join(', '), contextLabel: `${direction.integration} · ${direction.issueLabel || direction.sourceStatus} · ${direction.barcode}`, incidentTitle: direction.incidentTitle || direction.issueLabel || 'Инцидент GOVIN', issueClass: direction.issueClass, processStage: failedStage?.title || 'Проверка направления', externalDirectionId: direction.externalId || undefined, barcode: direction.barcode, recommendedRoute: direction.recommendedRoute, description: `GOVIN v0.5.9 · ${direction.issueLabel || direction.sourceStatus}. Идентификатор направления: ${direction.barcode}. Этап: ${failedStage?.title || 'Проверка направления'}. Клиент: ${direction.client || 'нет данных'}. ЛПУ: ${direction.lpu || 'нет данных'}. ИНЗ: ${direction.inz.join(', ') || 'не присвоен / нет данных'}. Интеграция: ${direction.integration}. ${direction.incidentDescription || ''} Рекомендованный маршрут: ${direction.recommendedRoute || 'уточнить по регламенту'}.` });
  };

  const createNotFoundIncident = () => {
    if (!integration || !barcode.trim()) return;
    onCreateIncident({ source: 'GOVIN-303', vendor: integration, inz: '', contextLabel: `${integration} · направление не найдено · ${barcode.trim()}`, incidentTitle: 'Направление не найдено в БД', issueClass: 'DATABASE_NOT_FOUND', processStage: 'Проверка направления', barcode: barcode.trim(), recommendedRoute: 'Техническая поддержка / ГСТИ', description: `GOVIN v0.5.9 · Направление не найдено в базе данных. Идентификатор: ${barcode.trim()}. Интеграция: ${integration}. Клиент, ЛПУ и ИНЗ в GOVIN не определены. Требуется проверить идентификатор, интеграцию и наличие данных в базе. Рекомендованный маршрут: техническая поддержка / ГСТИ.` });
  };

  const activeStage = result?.stages.find((stage) => stage.id === expandedStage) || null;

  return <div className="max-w-6xl mx-auto space-y-3 py-2 pb-10" translate="no">
    <header className="border-b border-slate-200 pb-4 flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
      <div>
        <h1 className="text-2xl sm:text-[28px] font-extrabold tracking-tight text-slate-900 leading-tight">Проверка направления и маппинга</h1>
        <p className="text-sm text-slate-500 mt-1.5">Найдите направление — GOVIN покажет, где возникла проблема и что делать дальше.</p>
      </div>
      <div className="flex items-center gap-2 sm:justify-end">
        <button type="button" onClick={() => setShowDemo((v) => !v)} className="hidden sm:inline-flex px-2 py-1.5 text-xs font-bold text-slate-500 hover:text-[#007f89]">{showDemo ? 'Скрыть DEMO' : 'DEMO-сценарии'}</button>
        <span className="px-2.5 py-1.5 rounded-md bg-[#e8f8f8] text-[#075b61] text-[11px] font-extrabold tracking-wide whitespace-nowrap">GOVIN-303 · DEMO v0.5.9</span>
      </div>
    </header>

    {showDemo && <div className="hidden sm:flex flex-wrap gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2">{(Object.keys(scenarioLabels) as DemoScenarioId[]).map((id) => <button key={id} type="button" onClick={() => selectScenario(id)} className="px-3 py-1.5 rounded-full border border-slate-200 bg-slate-50 hover:bg-slate-100 text-xs font-semibold">{id} · {scenarioLabels[id]}</button>)}</div>}

    <section aria-label="Проверка направления" className="bg-white rounded-xl border border-slate-200 p-4 sm:p-[18px] shadow-xs">
      <div className="text-[13px] font-extrabold text-slate-700 mb-3">Проверить направление</div>
      <div className="grid grid-cols-1 md:grid-cols-[270px_minmax(0,1fr)_220px] gap-3 items-start">
        <label>
          <span className="text-xs font-bold text-slate-700 block mb-1.5">Интеграция</span>
          <select ref={integrationRef} value={integration} onChange={(e) => { const next = e.target.value as Integration | ''; setIntegration(next); clearResult(); if (next) requestAnimationFrame(() => barcodeRef.current?.focus()); }} className="w-full h-[46px] px-3 rounded-lg border border-slate-200 bg-white text-sm"><option value="">Выберите интеграцию</option>{integrations.map((item) => <option key={item}>{item}</option>)}</select>
          <span className="block text-[11px] text-slate-400 mt-1.5">Источник, из которого пришло направление</span>
        </label>
        <label>
          <span className="text-xs font-bold text-slate-700 block mb-1.5">Номер направления / штрихкод</span>
          <input ref={barcodeRef} value={barcode} onChange={(e) => { setBarcode(e.target.value); clearResult(); }} onKeyDown={(e) => { if (e.key === 'Enter') handleSearch(); }} placeholder="Введите номер из алерта или чат-бота" className="w-full h-[46px] px-3 rounded-lg border border-slate-200 bg-white text-sm" />
          <span className={`block text-[11px] mt-1.5 ${integration && !barcode ? 'text-[#007f89] font-semibold' : 'text-slate-400'}`}>{integration ? `Введите номер для ${integration} — поле готово к вводу` : 'Можно ввести номер вручную или отсканировать штрихкод'}</span>
        </label>
        <div>
          <span className="text-xs font-bold text-transparent block mb-1.5 select-none" aria-hidden="true">Действие</span>
          <button type="button" onClick={handleSearch} className="w-full h-[46px] px-4 rounded-lg bg-[#00a6ad] hover:bg-[#007f89] text-white text-sm font-extrabold">Проверить направление</button>
          <span className="block text-[11px] text-slate-400 mt-1.5">После проверки появятся статус и действие ДКП</span>
        </div>
      </div>
      {validationMessage && <div role="alert" className="mt-2 text-xs font-semibold text-red-600">{validationMessage}</div>}
      <div className="mt-3 pt-3 border-t border-slate-100 flex items-start gap-2 text-[11px] text-slate-500"><span className="w-1.5 h-1.5 rounded-full bg-slate-300 mt-1 shrink-0"/><span>Бизнес-этапы «Создание / Чекин / Доставка» появляются только после проверки направления.</span></div>
    </section>

    {notFound && <div className="bg-white rounded-xl border border-amber-200 p-5 shadow-xs"><div className="flex gap-3"><CircleHelp className="w-5 h-5 text-amber-600 mt-0.5"/><div className="flex-1"><h2 className="font-extrabold text-slate-900">Направление не найдено</h2><p className="text-sm text-slate-600 mt-1">Проверьте идентификатор и выбранную интеграцию. Если данных нет в базе, эскалируйте в техническую поддержку.</p><div className="mt-3 text-xs rounded-lg border border-amber-200 bg-amber-50 p-3"><strong>Маршрут:</strong> техническая поддержка / ГСТИ.</div><ActionHint text="Откроется форма ПРИИЗ. Интеграция, идентификатор и причина «не найдено» будут перенесены автоматически." className="mt-4"><button type="button" onClick={createNotFoundIncident} className="px-4 py-2 rounded-lg border border-[#8bd2d6] bg-white text-[#007f89] font-bold text-sm inline-flex items-center gap-2 hover:bg-[#e9f8f8]">Создать обращение в ПРИИЗ <ArrowRight className="w-4 h-4"/></button></ActionHint></div></div></div>}

    {result && <>
      <div className={`rounded-xl border px-4 py-3 ${result.issueLabel ? 'border-red-200 bg-red-50' : 'border-emerald-200 bg-emerald-50'}`}><div className="flex items-start gap-3">{result.issueLabel ? <AlertTriangle className="w-5 h-5 text-red-600 mt-0.5 shrink-0"/> : <CheckCircle2 className="w-5 h-5 text-emerald-600 mt-0.5 shrink-0"/>}<div><div className="text-[11px] font-black uppercase tracking-wide text-slate-600">Итог проверки</div><h2 className="font-extrabold text-slate-900 mt-0.5">{result.issueLabel || 'Проблем не обнаружено'}</h2><p className="text-sm text-slate-700 mt-1">{result.stages.find((s) => s.state === 'error')?.action || 'Направление прошло доступные контрольные этапы. Дополнительных действий не требуется.'}</p></div></div></div>

      <section aria-label="Данные направления" className="bg-white rounded-xl border border-slate-200 shadow-xs px-4 py-3"><div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-2 border-b border-slate-100"><div className="flex items-center gap-2 flex-wrap"><strong className="font-mono text-sm text-slate-900">{result.externalId}</strong><span className="text-xs text-slate-400">{result.integration}</span></div><span className="px-2.5 py-1 rounded-full bg-slate-100 text-slate-700 text-[11px] font-bold">{result.sourceStatus}</span></div><dl className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-5 gap-y-2 pt-3 text-sm"><div><dt className="text-[11px] text-slate-400">Клиент / ЛПУ</dt><dd className="font-bold text-slate-800">{result.client} <span className="font-normal text-slate-500">· {result.lpu}</span></dd></div><div><dt className="text-[11px] text-slate-400">Создано / ИНЗ</dt><dd className="font-bold text-slate-800">{result.createdAt} <span className="font-normal text-slate-500">· {result.inz.join(', ') || 'Не присвоен'}</span></dd></div><div><dt className="text-[11px] text-slate-400">Услуга / штрихкод</dt><dd className="font-bold text-slate-800">{result.serviceName} <span className="font-mono font-normal text-slate-500">· {result.barcode}</span></dd></div><div><dt className="text-[11px] text-slate-400">База данных</dt><dd className="font-bold text-slate-800">{result.databaseState}</dd></div></dl></section>

      <section aria-label="Этапы проверки" className="bg-white rounded-xl border border-slate-200 shadow-xs p-3"><div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 mb-2"><h3 className="text-sm font-extrabold text-slate-900">Этапы процесса</h3><span className="text-[11px] text-slate-500">Нажмите на этап — откроются проверка, результат и действие ДКП</span></div><div className="grid grid-cols-1 md:grid-cols-3 gap-2">{result.stages.map((stage) => <button key={stage.id} type="button" aria-expanded={expandedStage === stage.id} aria-controls={`stage-detail-${stage.id}`} onClick={() => setExpandedStage((current) => current === stage.id ? null : stage.id)} className={`w-full text-left rounded-lg border px-3 py-2.5 transition ${stageStyles[stage.state]} hover:shadow-sm focus:outline-none focus:ring-2 focus:ring-[#0099a8]/30`}><div className="flex items-center justify-between gap-2"><span className="text-[11px] font-black text-slate-600">Этап {stage.number}</span><span className={`text-[10px] font-black uppercase tracking-wide ${stage.state === 'ok' ? 'text-emerald-700' : stage.state === 'error' ? 'text-red-700' : 'text-slate-500'}`}>{stageStatus[stage.state]}</span></div><div className="font-extrabold text-sm text-slate-900 leading-tight mt-1">{stage.title}</div><div className="text-[11px] text-slate-500 mt-1">Контроль: {stage.mappingType}</div><div className="flex items-center justify-end gap-1 text-[11px] font-bold text-[#007f89] mt-2">{expandedStage === stage.id ? 'Скрыть детали' : 'Открыть детали'}<ChevronDown className={`w-4 h-4 shrink-0 transition-transform ${expandedStage === stage.id ? 'rotate-180' : ''}`}/></div></button>)}</div>{activeStage && <div id={`stage-detail-${activeStage.id}`} className={`mt-2 rounded-lg border px-3 py-3 ${stageStyles[activeStage.state]}`}><div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs"><div><div className="font-black uppercase tracking-wide text-slate-500 text-[10px]">Что проверяем</div><div className="font-bold text-slate-800 mt-1">{activeStage.mappingType}</div></div><div><div className="font-black uppercase tracking-wide text-slate-500 text-[10px]">Что увидели</div><div className="text-slate-700 mt-1">{activeStage.summary}</div></div><div><div className="font-black uppercase tracking-wide text-slate-500 text-[10px]">Действие ДКП</div><div className="text-slate-700 mt-1">{activeStage.action}</div></div></div></div>}</section>

      <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-xs"><div className="flex items-center gap-2 mb-3"><Database className="w-5 h-5 text-[#00a6ad]"/><h2 className="font-extrabold text-slate-900">Сводка маппинга</h2></div><div className="overflow-x-auto"><table className="w-full text-xs"><thead><tr className="text-left border-b border-slate-200"><th className="py-2 pr-3">Тип</th><th className="py-2 pr-3">Код INVITRO / НМУ</th><th className="py-2 pr-3">Название INVITRO</th><th className="py-2 pr-3">Код вендора</th><th className="py-2 pr-3">Название в МИС/ЛИС</th><th className="py-2">Статус</th></tr></thead><tbody>{result.mappingRows.map((row, i) => <tr key={`${row.kind}-${i}`} className="border-b border-slate-100"><td className="py-3 pr-3 font-bold">{row.kind}</td><td className="py-3 pr-3 font-mono">{row.invitroCode}</td><td className="py-3 pr-3">{row.invitroName}</td><td className="py-3 pr-3 font-mono">{row.vendorCode}</td><td className="py-3 pr-3">{row.vendorName}</td><td className="py-3"><span className={`px-2 py-1 rounded-full font-bold ${row.status === 'Замапплено' ? 'bg-emerald-50 text-emerald-700' : row.status === 'Нет данных' ? 'bg-slate-100 text-slate-600' : 'bg-red-50 text-red-700'}`}>{row.status}</span></td></tr>)}</tbody></table></div></div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-3"><div className="bg-white rounded-xl border border-slate-200 p-4"><div className="flex items-center gap-2"><FlaskConical className="w-5 h-5 text-[#00a6ad]"/><h3 className="font-extrabold">Тесты и результаты</h3></div><div className="mt-3 text-xs"><strong>Назначенные тесты:</strong> {result.assignedTests.join(', ') || 'Нет данных'}</div><div className="mt-2 text-xs"><strong>Доставленные результаты:</strong> {result.deliveredResults.join(', ') || (result.scenario === 'S4' ? 'Не доставлены — доставка отменена' : 'Нет данных')}</div></div><div className="bg-white rounded-xl border border-slate-200 p-4"><h3 className="font-extrabold">Что делать дальше</h3><p className="text-sm text-slate-700 mt-2">{result.recommendedRoute ? <>Рекомендованный маршрут: <strong>{result.recommendedRoute}</strong>.</> : 'Регламентных действий не требуется.'}</p>{result.issueLabel && <ActionHint text="Откроется форма ПРИИЗ. Из GOVIN автоматически перенесутся известные данные направления, интеграция и причина проблемы." className="mt-4"><button type="button" onClick={() => createIncident(result)} className="px-4 py-2 rounded-lg border border-[#8bd2d6] bg-white text-[#007f89] font-bold text-sm inline-flex items-center gap-2 hover:bg-[#e9f8f8]">Создать обращение в ПРИИЗ <ArrowRight className="w-4 h-4"/></button></ActionHint>}</div></div>
    </>}

    <details className="rounded-lg border border-slate-100 bg-slate-50/60 px-3 py-2 text-[10px] text-slate-400"><summary className="cursor-pointer select-none">О DEMO</summary><div className="pt-2">Регламент основан на валидации команды ДКП от 21.08.2026. Используются только вымышленные данные. Точные production API, состав mapping-полей и маршруты команд остаются TBD до Technical Discovery.</div></details>
  </div>;
};