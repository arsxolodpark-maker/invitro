...
const scenarios: DirectionScenario[] = [
  {
    ...
    integration: 'Нетрика',
    barcode: '1236514265',
    externalId: 'DIR-DEMO-001',
    client: 'Демо-клиент 1',
    clientCode: 'CLI-DEMO-01',
    lpu: 'ЛПУ · DEMO 01',
    createdAt: '21.08.2026 09:15',
    inz: '942476082',
    service: 'Профиль обследования · DEMO',
    status: 'Доставка завершена',
    database: 'Направление и заказ найдены',
    issue: '',
  ...
  },
  {
    ...
    integration: 'Адыгея',
    barcode: '2236514265',
    externalId: 'DIR-DEMO-002',
    client: 'Демо-клиент 2',
    clientCode: 'CLI-DEMO-02',
    lpu: 'ЛПУ · DEMO 02',
    createdAt: '21.08.2026 09:40',
    inz: '',
    service: 'Услуга обследования · DEMO',
    status: 'Направление получено',
    database: 'Направление найдено, внутренний заказ ещё не создан',
    issue: 'Нет маппинга услуги',
    route: 'Сопровождение маппинга',
    userTitle: 'Услуга не сопоставлена',
    userContext: 'На этапе создания направления для одной услуги отсутствует маппинг.',
    userAction: 'Создайте обращение в ПРИИЗ на исправление маппинга услуги до чекина.',
    assignedTests: 'Нет данных',
    deliveredTests: 'Нет данных',
    stages: [
    ...
    mapping: [
      { type: 'Услуга', invitroCode: 'NMU-2001', invitroName: 'Услуга INVITRO · DEMO', vendorCode: 'V-SVC-88', vendorName: 'Услуга в МИС/ЛИС', status: 'Нет маппинга' },
    ],
  },
  {
    key: 'S3',
    integration: 'Брегис',
    barcode: '3236514265',
    externalId: 'DIR-DEMO-003',
    client: 'Демо-клиент 3',
    clientCode: 'CLI-DEMO-03',
    lpu: 'ЛПУ · DEMO 03',
    createdAt: '21.08.2026 10:05',
    inz: '942476083',
    service: 'Услуга обследования · DEMO',
    status: 'Нечекин',
    database: 'Направление и внутренний заказ найдены',
    issue: 'Нечекин / нет маппинга услуги',
    route: 'Исправление маппинга услуги + ручное лабораторное исполнение',
    userTitle: 'Направление не прошло чекин',
    userContext: 'Причина: на чекине отсутствует маппинг услуги.',
    userAction: 'Создайте обращение в ПРИИЗ: исправить маппинг услуги и передать заявку на ручное лабораторное исполнение.',
    assignedTests: 'Тест C · DEMO',
    deliveredTests: 'Нет данных',
    stages: [
    ...
    mapping: [
      { type: 'Услуга', invitroCode: 'NMU-3001', invitroName: 'Услуга INVITRO · DEMO', vendorCode: 'V-SVC-99', vendorName: 'Услуга в МИС/ЛИС', status: 'Нет маппинга' },
    ],
  },
  {
    key: 'S4',
    integration: 'Нетрика',
    barcode: '4236514265',
    externalId: 'DIR-DEMO-004',
    client: 'Демо-клиент 4',
    clientCode: 'CLI-DEMO-04',
    lpu: 'ЛПУ · DEMO 04',
    createdAt: '21.08.2026 10:30',
    inz: '942476084',
    service: 'Профиль обследования · DEMO',
    status: 'Доставка отменена',
    database: 'Направление, заказ и результаты найдены; доставка не выполнена',
    issue: 'Ошибка доставки / нет маппинга теста',
    route: 'ГСТИ — если интеграция на поддержке; чат «Ежевика» — если интеграция ещё на сопровождении разработки',
    userTitle: 'Результаты не доставлены',
    userContext: 'Причина: у одного теста отсутствует маппинг. Доставка результатов отменена.',
    userAction: 'Создайте обращение в ПРИИЗ на исправление маппинга теста и повторную отправку результата.',
    assignedTests: 'Тест D · качественный, Тест E · количественный',
    deliveredTests: 'Не доставлены — доставка отменена',
    stages: [
...
export const DirectionCheckView: React.FC<DirectionCheckViewProps> = ({ onCreateIncident }) => {
  ...
  const failedStageIndex = direction ? direction.stages.findIndex((stage) => stage.status === 'error') : -1;
  ...
  const handleSearch = () => {
    ...
    setScenario(found || { key: 'S5', integration, barcode: barcode.trim() });
    setDetailsOpen(false);
    setMappingOpen(false);
    setExpandedStage(null);
  };
  ...
  const routeText = scenario?.key === 'S5'
    ...
    : direction?.issue
      ? `Маршрут по регламенту: ${direction.route}.`
      : '';
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-4 py-4 pb-16 notranslate" translate="no">
      ...
      <section className="bg-white rounded-xl border border-slate-200 p-5 shadow-xs" aria-label="Проверка направления">
        ...
        <div className="grid grid-cols-1 md:grid-cols-[270px_1fr_230px] gap-3 items-start">
          <label className="text-sm font-bold text-slate-700">
            ...
            <select
              ...
              onChange={(e) => {
                ...
                setValidationMessage('');
                clearResult();
                if (e.target.value) setTimeout(() => barcodeRef.current?.focus(), 0);
              }}
          ...
          <div className="md:pt-[25px]">
            <button type="button" onClick={handleSearch} className="w-full h-12 rounded-lg bg-[#0099a8] hover:bg-[#007f89] text-white font-extrabold text-base inline-flex items-center justify-center gap-2">
              <Search className="w-4 h-4" /> Проверить направление
            </button>
            <span className="block mt-1 text-xs text-slate-500">Получите итог и следующий шаг</span>
          </div>
        </div>
          ...
          <div className="flex flex-wrap gap-2 justify-end mt-2">
            {scenarios.map((item) => (
              <button
                ...
                className="px-2.5 py-1.5 border border-slate-200 bg-slate-50 rounded-md text-slate-600 font-semibold"
              >
                {item.key} · {item.userTitle}
              </button>
            ))}
            <button
              type="button"
              onClick={() => {
                setIntegration('Нетрика');
                setBarcode('9999999999');
                setScenario({ key: 'S5', integration: 'Нетрика', barcode: '9999999999' });
                setDetailsOpen(false);
                setMappingOpen(false);
                setExpandedStage(null);
              }}
      ...
      {scenario && (
        <section
          aria-label="Итог и следующее действие"
          className={`rounded-xl border p-5 md:p-6 ${scenario.key === 'S5' ? 'bg-amber-50 border-amber-200' : direction?.issue ? 'bg-red-50 border-red-200' : 'bg-emerald-50 border-emerald-200'}`}
        >
          ...
          <div className="mt-5 bg-white/80 border border-slate-200/70 rounded-lg p-4">
            <div className="text-sm font-extrabold text-slate-600">Что делать сейчас</div>
            <p className="text-lg md:text-xl font-extrabold text-[#17383d] leading-snug mt-1">{actionNow}</p>
            {routeText && <p className="text-sm text-slate-600 mt-2">{routeText}</p>}
          </div>

          <div className="flex flex-col sm:flex-row gap-2 mt-5">
            {scenario.key === 'S1' && (
              <button type="button" onClick={resetSearch} className="h-12 px-5 rounded-lg bg-[#0099a8] hover:bg-[#007f89] text-white font-extrabold">
                Проверить другое направление
              </button>
            )}

            {scenario.key !== 'S1' && scenario.key !== 'S5' && (
              <button type="button" onClick={createIncident} className="h-12 px-5 rounded-lg bg-[#0099a8] hover:bg-[#007f89] text-white font-extrabold">
                Создать обращение в ПРИИЗ
              </button>
            )}
            ...
            {scenario.key !== 'S5' && (
              <button type="button" onClick={openDetails} className="h-12 px-5 rounded-lg border border-slate-300 bg-white text-slate-700 font-bold">
                {detailsOpen ? 'Скрыть детали проверки' : 'Показать детали проверки'}
              </button>
            )}
          </div>
        </section>
      ...
      {direction && detailsOpen && (
        <section aria-label="Детали проверки" className="space-y-3">
          <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
            <div className="p-4 border-b border-slate-100 flex flex-col sm:flex-row justify-between gap-2">
              <div>
                <div className="text-xs text-slate-500">{direction.integration} · внешнее направление</div>
                <div className="font-mono font-bold text-slate-900 break-all mt-1">{direction.externalId}</div>
              </div>
              <span className="self-start px-3 py-1 rounded-full bg-slate-100 text-slate-700 text-xs font-bold">{direction.status}</span>
            </div>

            <dl className="grid grid-cols-1 md:grid-cols-4 gap-4 p-4">
              <div>
                <dt className="text-xs text-slate-500">Клиент / ЛПУ</dt>
                <dd className="text-sm font-bold mt-1">{direction.client}<span className="block font-normal text-slate-500">{direction.lpu}</span></dd>
              </div>
              <div>
                <dt className="text-xs text-slate-500">Создано / ИНЗ</dt>
                <dd className="text-sm font-bold mt-1">{direction.createdAt}<span className="block font-normal text-slate-500">{direction.inz || 'Не присвоен'}</span></dd>
              </div>
              <div>
                <dt className="text-xs text-slate-500">Услуга / штрихкод</dt>
                <dd className="text-sm font-bold mt-1">{direction.service}<span className="block font-normal text-slate-500 break-all">{direction.barcode}</span></dd>
              </div>
              <div>
                <dt className="text-xs text-slate-500">База данных</dt>
                <dd className="text-sm font-bold mt-1">{direction.database}</dd>
              </div>
            </dl>
          </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-4">
            <div className="flex flex-col sm:flex-row justify-between gap-1 mb-3">
              <h3 className="font-extrabold text-[#17383d]">Этапы процесса</h3>
              <span className="text-xs text-slate-500">Нажмите этап, чтобы посмотреть подробности</span>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
              {direction.stages.map((stage, index) => (
                <button
                  key={stage.name}
                  type="button"
                  onClick={() => setExpandedStage(expandedStage === index ? null : index)}
                  className={`text-left p-3 rounded-lg border min-h-24 ${stage.status === 'error' ? 'bg-red-50 border-red-200' : stage.status === 'ok' ? 'bg-emerald-50 border-emerald-200' : 'bg-slate-50 border-slate-200'}`}
                >
                  <div className="flex justify-between gap-2 text-xs font-extrabold">
                    <span>Этап {index + 1}</span>
                    <span>{statusText(stage.status)}</span>
                  </div>
                  <div className="text-sm font-extrabold mt-2">{stage.name}</div>
                  <div className="text-xs text-slate-500 mt-1">{stage.control}</div>
                </button>
              ))}
            </div>

            {expandedStage !== null && (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 border-t border-slate-100 mt-3 pt-3">
                <div><b className="block text-xs uppercase text-slate-500 mb-1">Что проверяем</b><span className="text-sm">{direction.stages[expandedStage].control}</span></div>
                <div><b className="block text-xs uppercase text-slate-500 mb-1">Что увидели</b><span className="text-sm">{direction.stages[expandedStage].observed}</span></div>
                <div><b className="block text-xs uppercase text-slate-500 mb-1">Действие ДКП</b><span className="text-sm">{direction.stages[expandedStage].action}</span></div>
              </div>
            )}
          </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-4">
            <div className="flex flex-col sm:flex-row justify-between gap-3">
              <div>
                <h3 className="font-extrabold text-[#17383d]">Тесты и результаты</h3>
                <p className="text-sm mt-2 text-slate-600"><strong>Назначенные:</strong> {direction.assignedTests}</p>
                <p className="text-sm mt-1 text-slate-600"><strong>Доставленные:</strong> {direction.deliveredTests}</p>
              </div>
              <button
                type="button"
                onClick={() => setMappingOpen(!mappingOpen)}
                aria-expanded={mappingOpen}
                className="self-start h-11 px-4 rounded-lg border border-slate-300 bg-white text-slate-700 text-sm font-bold inline-flex items-center gap-2"
              >
                {mappingOpen ? 'Скрыть маппинг' : 'Показать маппинг'}
                {mappingOpen ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
              </button>
            </div>
                ...
                <table className="w-full text-xs min-w-[760px]">
                  <thead>
                    <tr className="bg-slate-50 text-slate-600">
                      ...
                      <th className="p-2 text-left">Код INVITRO / НМУ</th>
                      <th className="p-2 text-left">Название INVITRO</th>
                      <th className="p-2 text-left">Код вендора</th>
                      <th className="p-2 text-left">Название в МИС/ЛИС</th>
                      <th className="p-2 text-left">Статус</th>
                    </tr>
                  </thead>
                  <tbody>
                    {direction.mapping.map((row) => (
                      <tr key={`${row.type}-${row.invitroCode}`} className="border-b border-slate-100">
                        <td className="p-2 font-bold">{row.type}</td>
                        <td className="p-2">{row.invitroCode}</td>
                        <td className="p-2">{row.invitroName}</td>
                        <td className="p-2">{row.vendorCode}</td>
                        <td className="p-2">{row.vendorName}</td>
                        <td className={`p-2 font-bold ${row.status === 'Замапплено' ? 'text-emerald-700' : 'text-red-700'}`}>{row.status}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
