...
export const DirectionCheckView: React.FC<DirectionCheckViewProps> = ({ onCreateIncident }) => {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-3 py-2 pb-10 notranslate" translate="no">
      <div className="bg-white rounded-2xl border border-slate-200 px-5 py-4 shadow-xs">
        <div className="flex items-center gap-2 text-[#007f89] font-bold text-[11px] uppercase tracking-wider mb-1"><Waypoints className="w-4 h-4" /> GOVIN-303 · DEMO v0.5.1</div>
        <h1 className="text-xl sm:text-2xl font-extrabold text-slate-900 leading-tight">Проверка направления и маппинга</h1>
        <p className="text-xs sm:text-sm text-slate-500 mt-1">Получение услуг → чекин → доставка результатов</p>
      </div>

      <div className="bg-white rounded-2xl border border-slate-200 px-5 py-4 shadow-xs">
        <div className="grid grid-cols-1 md:grid-cols-[25fr_60fr_15fr] gap-3 items-end">
        ...
        <div className="mt-3 pt-3 border-t border-slate-100">
          <button type="button" onClick={() => setShowDemo((value) => !value)} className="text-xs font-bold text-slate-500 hover:text-[#007f89]">{showDemo ? 'Скрыть демо-сценарии' : 'Демо-сценарии'} {showDemo ? '↑' : '↓'}</button>
          {showDemo && <div className="flex flex-wrap gap-2 mt-2">{(Object.keys(scenarioLabels) as DemoScenarioId[]).map((id) => <button key={id} type="button" onClick={() => selectScenario(id)} className="px-3 py-1.5 rounded-full border border-slate-200 bg-slate-50 hover:bg-slate-100 text-xs font-semibold">{id} · {scenarioLabels[id]}</button>)}</div>}
        </div>
      </div>
      ...
      {result && (
        <>
          <div className={`rounded-2xl border p-4 ${result.issueLabel ? 'border-red-200 bg-red-50' : 'border-emerald-200 bg-emerald-50'}`}>
            <div className="flex items-start gap-3">
              ...
              <div>
                <div className="text-[11px] font-black uppercase tracking-wide text-slate-600">Итог проверки</div>
                <h2 className="font-extrabold text-slate-900 mt-0.5">{result.issueLabel || 'Проблем не обнаружено'}</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div className="bg-white rounded-2xl border border-slate-200 p-4"><div className="flex items-center gap-2"><FlaskConical className="w-5 h-5 text-[#0099a8]"/><h3 className="font-extrabold">Тесты и результаты</h3></div><div className="mt-3 text-xs"><strong>Назначенные тесты:</strong> {result.assignedTests.join(', ') || 'Нет данных'}</div><div className="mt-2 text-xs"><strong>Доставленные результаты:</strong> {result.deliveredResults.join(', ') || 'Нет данных'}</div></div>
            <div className="bg-white rounded-2xl border border-slate-200 p-4"><h3 className="font-extrabold">Что делать дальше</h3><p className="text-sm text-slate-700 mt-2">{result.recommendedRoute ? <>Рекомендованный маршрут: <strong>{result.recommendedRoute}</strong>.</> : 'Регламентных действий не требуется.'}</p>{result.issueLabel && <button type="button" onClick={() => createIncident(result)} className="mt-4 px-4 py-2 rounded-lg border border-[#8bd2d6] bg-white text-[#007f89] font-bold text-sm inline-flex items-center gap-2 hover:bg-[#e9f8f8]">Создать инцидент в ПРИИЗ <ArrowRight className="w-4 h-4"/></button>}</div>
          </div>
        </>
      )}

      <details className="rounded-xl border border-slate-100 bg-slate-50/60 px-3 py-2 text-[10px] text-slate-400">
        <summary className="cursor-pointer select-none">О DEMO</summary>
        <div className="pt-2">Регламент v0.5 основан на валидации команды ДКП от 21.08.2026. Используются только вымышленные данные. Тип промышленного поискового идентификатора и точные маршруты команд остаются TBD до сверки контрактов. Автоматическая маршрутизация, повторная отправка результатов и реальные API не выполняются.</div>
      </details>
    </div>
  );
};