    ...
    <header className="bg-white border-b border-[#dce9e9] sticky top-0 z-30 shadow-[0_2px_16px_rgba(24,74,79,0.05)]">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
          ...
          <div className="flex items-center bg-[#f4f8f8] p-1 rounded-xl border border-[#ddeaea] overflow-x-auto"><span className="text-xs font-semibold text-slate-500 px-2 flex items-center gap-1 shrink-0"><Shield className="w-3.5 h-3.5 text-[#0099a8]" />Контур:</span>{VISIBLE_ROLES.map((role) => <button key={role} onClick={() => onRoleChange(role)} className={`px-3 py-1.5 rounded-lg text-xs transition-all shrink-0 ${currentRole === role ? 'bg-[#0099a8] text-white shadow-sm font-bold' : 'text-slate-600 hover:text-[#007f89] hover:bg-white font-medium'}`}>{role}</button>)}</div>
        </div>
