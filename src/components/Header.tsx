...
const ROLES_INFO: Record<UserRole, { label: string; desc: string; icon: React.ComponentType<{ className?: string }> }> = {
  ...
  'Инженер ГСТИ': { label: 'Инженер ГСТИ', desc: 'Обработка обращения в 1C:ITILIUM; портал показывает контекст и обмен', icon: Headphones },
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
          ...
          <button className="flex items-center gap-3 text-left min-w-0" onClick={() => onNavigate('home')}>
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                <span className="font-extrabold text-[#16383d] tracking-tight text-lg">{isExternal ? 'ПРИИЗ' : 'Рабочий портал'}</span>
                <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-[#e9f8f8] text-[#007f89] border border-[#bce8e8] uppercase tracking-wide">v0.7 demo</span>
              </div>
              <p className="text-xs text-slate-500 truncate">{isExternal ? 'Портал для Регистрации Инцидентов в Интеграциях Здравоохранения' : 'ПРИИЗ - модуль «Инциденты» единого фронта ДКП'}</p>
            </div>
          </button>

          <div className="flex items-center bg-[#f4f8f8] p-1 rounded-xl border border-[#ddeaea] overflow-x-auto">
            <span className="text-xs font-semibold text-slate-500 px-2 flex items-center gap-1 shrink-0"><Shield className="w-3.5 h-3.5 text-[#0099a8]" />Контур:</span>
        ...
        {currentRole === 'ДКП' && (
          <div className="mt-3 flex items-center gap-1 overflow-x-auto border-t border-[#edf3f3] pt-2.5">
            ...
            {PRODUCT_TABS.map(({ label, view, icon: Icon, active }) => (
              <button key={label} disabled={!active} onClick={() => active && view && onNavigate(view)} title={active ? '' : 'Отдельный продукт - будет подключен позже'} className={`shrink-0 px-3 py-1.5 rounded-lg text-xs font-semibold flex items-center gap-1.5 border ${active && ((view === 'home' && activeView !== 'direction-check') || activeView === view) ? 'bg-[#e9f8f8] text-[#007f89] border-[#bce8e8]' : active ? 'bg-white text-slate-600 border-[#dfeaea] hover:bg-[#f5fbfb]' : 'bg-[#fafcfc] text-slate-400 border-[#edf2f2] cursor-not-allowed'}`}><Icon className="w-3.5 h-3.5" />{label}{!active && <span className="text-[9px] uppercase">later</span>}</button>
            ))}
          </div>
        )}
        ...
        <div className="mt-2.5 pt-2.5 border-t border-[#edf3f3] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 text-xs">
          ...
          <div className="flex items-center space-x-1 self-end sm:self-auto shrink-0">
            {!isExternal && currentRole !== 'Администратор' && <button onClick={() => onNavigate('home')} className={navClass(activeView === 'home')}><Activity className="w-3.5 h-3.5" /><span>Инциденты</span></button>}
            ...
            {currentRole === 'Администратор' && <button onClick={() => onNavigate('admin')} className={navClass(activeView === 'admin')}><Users className="w-3.5 h-3.5" /><span>Пользователи / Express</span></button>}
