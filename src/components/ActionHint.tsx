...
export const ActionHint: React.FC<ActionHintProps> = ({
  text,
  children,
  mobilePrefix = 'После нажатия:',
  className = '',
  align = 'center',
  overlay,
}) => {
  // Search uses the full-width wrapper and keeps the compact popover.
  // Transition actions use a stable inline helper strip, so hover never
  // changes button geometry and cannot swallow a click.
  const useOverlay = overlay ?? className.split(/\s+/).includes('w-full');

  return (
    <div className={`group relative inline-flex max-w-full flex-col items-start ${useOverlay ? '' : 'md:w-full'} ${className}`}>
      {children}
      {useOverlay ? (
        <div
          role="tooltip"
