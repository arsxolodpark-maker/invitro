...
const DEMO_PASSWORD = 'DemoPassword!1';
...
export const InitiatorPortalView: React.FC<InitiatorPortalViewProps> = ({ incidents, onCreateIncident, onSelectIncident }) => {
  ...
  const [activationPassword, setActivationPassword] = useState(DEMO_PASSWORD);
  ...
  const activate = () => {
    ...
    const updated = activateInitiator(account.id);
    setAccount(updated);
    setError('');
    setStep('login');
  };
  ...
  const login = () => {
    ...
    // В публичном DEMO пароль не сохраняется и не проверяется сервером.
    startInitiatorSession(account.id);
    setError('');
    setStep('portal');
  };
    ...
    return (
      <div className="max-w-xl mx-auto py-10 notranslate" translate="no">
        <div className="bg-white border border-[#dfeaea] rounded-2xl p-7 shadow-sm space-y-5">
          ...
          <div><div className="text-xs font-bold uppercase tracking-wider text-[#008c98]">Внешний контур ПРИИЗ</div><h1 className="text-2xl font-extrabold text-[#17383d] mt-1">Активация аккаунта</h1><p className="text-sm text-slate-600 mt-2">ДКП создал учетную запись. Проверьте данные и задайте пароль.</p></div>
          ...
          <label className="block"><span className="text-xs font-bold text-slate-700">Создайте пароль</span><div className="mt-1 relative"><KeyRound className="w-4 h-4 text-slate-400 absolute left-3 top-3" /><input aria-label="Пароль для активации" type="password" value={activationPassword} onChange={(e) => { setActivationPassword(e.target.value); setError(''); }} className="w-full pl-9 pr-3 py-2.5 rounded-xl border border-[#d8e7e7] bg-white text-sm" /></div></label>
          {error && <div role="alert" className="text-xs font-semibold text-red-600">{error}</div>}
          <button type="button" onClick={activate} className="w-full py-3 rounded-xl bg-[#0099a8] hover:bg-[#008891] text-white font-bold text-sm">Активировать аккаунт</button>
          <div className="text-[11px] text-slate-400">DEMO: пароль не отправляется и не сохраняется на сервере.</div>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="max-w-md mx-auto py-12 notranslate" translate="no">
        <div className="bg-white border border-[#dfeaea] rounded-2xl p-7 shadow-sm space-y-5">
          ...
          <label className="block"><span className="text-xs font-bold text-slate-700">Email</span><input aria-label="Email" value={account.email} readOnly className="mt-1 w-full px-3 py-2.5 rounded-xl border border-[#d8e7e7] bg-[#f8fbfb] text-sm" /></label>
          <label className="block"><span className="text-xs font-bold text-slate-700">Пароль</span><input aria-label="Пароль" type="password" value={loginPassword} onChange={(e) => { setLoginPassword(e.target.value); setError(''); }} className="mt-1 w-full px-3 py-2.5 rounded-xl border border-[#d8e7e7] text-sm" /></label>
          {error && <div role="alert" className="text-xs font-semibold text-red-600">{error}</div>}
          <button type="button" onClick={login} className="w-full py-3 rounded-xl bg-[#0099a8] hover:bg-[#008891] text-white font-bold text-sm">Войти</button>
          <div className="text-[11px] text-slate-400">DEMO: реальная аутентификация не выполняется.</div>
        </div>
      </div>
    );
  }
