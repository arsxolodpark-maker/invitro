...
export const IncidentDetailCard: React.FC<IncidentDetailCardProps> = ({ incident, currentRole, onBack, onOpenConsole, onAddComment, onConfirmReceipt, onCloseIncident, onStatusChange }) => {
  ...
  const canConfirm = (currentRole === 'Инициатор' || currentRole === 'ДКП') && ['В работе', 'Ожидает ответа', 'Ожидает согласования', 'Выполнено'].includes(incident.status) && incident.status !== 'Закрыт';
  const canClose = isEngineer && incident.status === 'Выполнено';
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-5 py-4 pb-16">
      ...
      <section className="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs space-y-5">
        ...
        {isInitiator ? <div className="p-4 rounded-xl bg-[#f3fbfb] border border-[#cfeaea] text-xs"><div className="font-bold text-[#17383d]">Что происходит сейчас</div><p className="text-slate-600 mt-1">Статус обращения синхронизируется с внутренним Service Desk. Вам не нужно знать внутренние технические статусы - ПРИИЗ показывает их в понятной модели.</p></div> : <div className="p-4 rounded-xl bg-[#f3fbfb] border border-[#cfeaea] text-xs"><div className="font-bold text-[#17383d]">Подтвержденный контур 1C:ITILIUM</div><p className="text-slate-600 mt-1">v9 подтверждает создание обращений, получение обращений и статусов. Точный production-контракт двусторонней синхронизации комментариев остается TBD.</p></div>}
      ...
      <section className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs flex flex-col sm:flex-row justify-between gap-4"><div className="text-xs text-slate-600">{isInitiator ? 'После решения подтвердите, что результат получен. После этого инженер сможет окончательно закрыть обращение.' : <><strong>Express:</strong> DEMO-событие об изменении обращения отправляется в настроенный администратором чат. Реального вызова Express нет.</>}</div><div className="flex gap-2 justify-end">{canConfirm && incident.status !== 'Выполнено' && <button onClick={() => onConfirmReceipt(incident.id)} className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-[#0099a8] text-white text-xs font-bold"><CheckCircle2 className="w-4 h-4" />Подтвердить получение результата</button>}{canClose && <button onClick={() => setShowClose(true)} className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-slate-900 text-white text-xs font-bold"><Tag className="w-4 h-4" />Закрыть обращение</button>}</div></section>
