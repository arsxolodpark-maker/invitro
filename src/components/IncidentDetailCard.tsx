  ...
  return (
    <div className="max-w-5xl mx-auto space-y-5 py-4 pb-16">
      ...
      <section className="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs space-y-5">
        ...
        {isInitiator ? <div className="p-4 rounded-xl bg-[#f3fbfb] border border-[#cfeaea] text-xs"><div className="font-bold text-[#17383d]">Что происходит сейчас</div><p className="text-slate-600 mt-1">ПРИИЗ показывает понятный статус обращения. Внутренние технические статусы и служебные действия 1C:ITILIUM пользователю не показываются.</p>{incident.status === 'Выполнено' && !incident.resultConfirmed && <p className="mt-2 font-semibold text-[#007f89]">Инженер завершил работы. Проверьте результат и подтвердите получение.</p>}{incident.resultConfirmed && incident.status !== 'Закрыт' && <p className="mt-2 font-semibold text-emerald-700">Получение подтверждено. Ожидается финальное закрытие обращения инженером.</p>}</div> : <div className="p-4 rounded-xl bg-[#f3fbfb] border border-[#cfeaea] text-xs"><div className="font-bold text-[#17383d]">Подтвержденный контур 1C:ITILIUM</div><p className="text-slate-600 mt-1">v9 подтверждает создание обращений, получение обращений и статусов. Точный production-контракт двусторонней синхронизации комментариев остается TBD.</p></div>}
      ...
      <section className="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs space-y-4">
        ...
        {incident.status !== 'Закрыт' && <form onSubmit={submitComment} className="flex gap-2"><input value={commentText} onChange={(e) => setCommentText(e.target.value)} placeholder={isEngineer ? 'Ответ пользователю или комментарий по обращению...' : 'Добавить комментарий...'} className="flex-1 px-3 py-2.5 rounded-xl border border-slate-200 text-sm" /><button className="px-4 rounded-xl bg-[#0099a8] text-white"><Send className="w-4 h-4" /></button></form>}
      </section>

      <section className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs flex flex-col sm:flex-row justify-between gap-4"><div className="text-xs text-slate-600">{isInitiator ? 'Подтверждение результата - отдельное пользовательское действие перед финальным закрытием.' : <><strong>Express:</strong> DEMO-событие об изменении обращения отправляется в настроенный администратором чат. Реального вызова Express нет.</>}</div><div className="flex gap-2 justify-end">{canConfirm && <button onClick={() => onConfirmReceipt(incident.id)} className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-[#0099a8] text-white text-xs font-bold"><CheckCircle2 className="w-4 h-4" />Подтвердить получение результата</button>}{canClose && <button onClick={() => setShowClose(true)} className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-slate-900 text-white text-xs font-bold"><Tag className="w-4 h-4" />Закрыть обращение</button>}</div></section>

      {showClose && <div className="fixed inset-0 z-50 bg-slate-950/40 flex items-center justify-center p-4"><form onSubmit={close} className="w-full max-w-lg bg-white rounded-2xl p-6 shadow-2xl space-y-4"><div><h3 className="font-bold text-lg">Закрытие обращения</h3><p className="text-xs text-slate-500">Причина и решение сохраняются для анализа и будущей базы знаний.</p></div><input value={rootCause} onChange={(e) => setRootCause(e.target.value)} className="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm" /><textarea value={resolution} onChange={(e) => setResolution(e.target.value)} className="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm min-h-24" /><div className="flex justify-end gap-2"><button type="button" onClick={() => setShowClose(false)} className="px-4 py-2 text-sm">Отмена</button><button className="px-4 py-2 rounded-xl bg-slate-900 text-white text-sm font-bold">Закрыть</button></div></form></div>}
    </div>
  );
};
