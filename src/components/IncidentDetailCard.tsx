...
export const IncidentDetailCard: React.FC<IncidentDetailCardProps> = ({ incident, currentRole, onBack, onOpenConsole, onAddComment, onConfirmReceipt, onCloseIncident, onStatusChange }) => {
  ...
  const canConfirm = isInitiator && incident.status === 'Выполнено' && !incident.resultConfirmed;
  ...
  const close = (e: React.FormEvent) => {
    ...
    onCloseIncident(incident.id, rootCause.trim(), resolution.trim());
    setShowClose(false);
  };

  return (
    <div className="max-w-5xl mx-auto space-y-5 py-4 pb-16 notranslate" translate="no">
      <button type="button" onClick={onBack} className="inline-flex items-center gap-2 text-sm font-semibold text-slate-600 bg-white px-3 py-1.5 rounded-lg border border-slate-200"><ArrowLeft className="w-4 h-4" />К списку</button>
      ...
      <section className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs flex flex-col sm:flex-row justify-between gap-4"><div className="text-xs text-slate-600">{isInitiator ? 'После выполнения подтвердите, что результат получен.' : <><strong>Express:</strong> уведомление об изменении обращения · DEMO.</>}</div><div className="flex gap-2 justify-end">{canConfirm && <button type="button" onClick={() => onConfirmReceipt(incident.id)} className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-[#0099a8] text-white text-xs font-bold"><CheckCircle2 className="w-4 h-4" />Подтвердить получение результата</button>}{canClose && <button type="button" onClick={() => setShowClose(true)} className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-slate-900 text-white text-xs font-bold"><Tag className="w-4 h-4" />Закрыть обращение</button>}</div></section>

      {showClose && <div className="fixed inset-0 z-50 bg-slate-950/40 flex items-center justify-center p-4"><form onSubmit={close} className="w-full max-w-lg bg-white rounded-2xl p-6 shadow-2xl space-y-4"><div><h3 className="font-bold text-lg">Закрытие обращения</h3><p className="text-xs text-slate-500">Зафиксируйте причину и решение для последующего анализа.</p></div><label className="block text-xs font-bold text-slate-700">Причина<input required value={rootCause} onChange={(e) => setRootCause(e.target.value)} className="mt-1 w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm font-normal" /></label><label className="block text-xs font-bold text-slate-700">Решение<textarea required value={resolution} onChange={(e) => setResolution(e.target.value)} className="mt-1 w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm min-h-24 font-normal" /></label><div className="flex justify-end gap-2"><button type="button" onClick={() => setShowClose(false)} className="px-4 py-2 text-sm">Отмена</button><button className="px-4 py-2 rounded-xl bg-slate-900 text-white text-sm font-bold">Закрыть</button></div></form></div>}
    </div>
  );
};
