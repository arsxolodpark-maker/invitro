    ...
    <div className="max-w-3xl mx-auto py-4 pb-16 space-y-5 notranslate" translate="no">
      ...
      <form onSubmit={submit} className="bg-white border border-[#dfeaea] rounded-2xl p-6 shadow-xs space-y-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <label><span className="text-xs font-bold text-slate-700">ИНЗ / номер заявки *</span><input value={inz} onChange={(e) => setInz(e.target.value)} className={common} /></label>
          ...
          <label className="md:col-span-2"><span className="text-xs font-bold text-slate-700">Описание проблемы *</span><textarea value={description} onChange={(e) => setDescription(e.target.value)} className={`${common} min-h-24`} /></label>
        </div>
